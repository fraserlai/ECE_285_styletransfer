...
def subtract_imagenet_mean_batch(batch, device):
    ...
    mean[:, 1, :, :] = 116.779
    mean[:, 2, :, :] = 123.680
    return batch.to(device) - Variable(mean).to(device)

def add_imagenet_mean_batch(batch, device):
...
class StyleLoader():
    def __init__(self, style_folder, style_size, device):
    ...
    def get(self, i):
        ...
        style_v = Variable(style, requires_grad=False).to(self.device)
        return style_v
